...
| [Steam Support: Steam Guard mobile authenticator setup](https://help.steampowered.com/en/faqs/view/6891-E071-C9D9-0134) | Official setup recommends a phone number for recovery, but explicitly says users without a phone number can select "I don't have access to a phone number" and still add the authenticator. |
...
| [xPaw Steam Web API: `ITwoFactorService`](https://steamapi.xpaw.me/ITwoFactorService) | Reference for `AddAuthenticator`, `FinalizeAddAuthenticator`, and `SendEmail`; `sms_phone_id` is optional, `activation_code` is an out-of-band code, `validate_sms_code` is only for SMS finalization, and `SendEmail` can include the activation code. |
...
1. Use a deliberately chosen disposable/low-value account for the first test. A phone number is useful for recovery, but Steam's official setup says users without one can choose the no-phone path and still add the authenticator.
...
3. Start enrollment with Steam's `AddAuthenticator` equivalent using the account SteamID, current authenticator time, `ValveMobileApp` authenticator type, deterministic `device_identifier`, and token `version=2`. Do not link a phone number inside the helper; `sms_phone_id` is optional and applies only when Steam uses SMS verification.
...
5. Ask the user for Steam's activation code from email or SMS. If no code arrives, request an activation-code email through `ITwoFactorService/SendEmail` with `include_activation_code=true` or tell the user to use the official app's no-phone path.
6. Finalize with `FinalizeAddAuthenticator`, passing both the activation code and the current Steam TOTP code generated from the newly returned `shared_secret`. Only pass `validate_sms_code` when the supplied activation code is known to be an SMS code.
7. Handle Steam's `want_more`/time-skew retry behavior by advancing the authenticator timestamp in one 30-second step, as ValvePython does, but cap retries.
8. After finalization succeeds, show the revocation code once with a backup warning, then move all secrets into the Windows secret store and remove plaintext temporary files.
